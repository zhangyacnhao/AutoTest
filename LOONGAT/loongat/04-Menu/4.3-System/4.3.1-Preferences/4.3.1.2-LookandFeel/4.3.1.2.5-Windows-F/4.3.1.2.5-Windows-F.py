...
#########################################################################
#全局变量区域
g_tag ="4.3.1.2.5-Windows-F" #####
mylog = Logcase()
Sshot = Screenshot()
winName = "未命名-Windows-F" #####
global passwd
######################################################################
def chkdir(dir):
	try:
...
def Openwin():
	try: 
		pid = launchapp('mate-window-properties')  #####
  		print pid,type(pid)
...
def ChekAndClosWin():
	try:
		res = waittillguiexist('dlgWindowPreferences')
		print res
		if res==1:
			mylog.ilog(g_tag,'i catch the window')
			activatewindow('dlgWindowPreferences')
			sleep(3)
			Sshot.scrprint(g_tag, 'After_Windows-F', './')		
...
def CloeWin():
	try:
		...
		closewindow('dlgWindowPreferences')
		result = waittillguiexist('dlgWindowPreferences')
		if result==0:
			print mylog.ilog(g_tag,'closing window successifully!')			
		else:
			kcmd = "pkill mate-window"
			os.system(kcmd)
