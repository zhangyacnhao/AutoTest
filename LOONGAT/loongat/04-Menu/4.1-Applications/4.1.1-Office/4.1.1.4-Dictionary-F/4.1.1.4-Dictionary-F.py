...
#########################################################################
#全局变量区域
g_tag ="4.1.1.4-Dictionary-F"
mylog = Logcase()
Sshot = Screenshot()
winName = "未命名-Dictionary-F"
global passwd
def chkdir(dir):
	try:
		...
		return True
def work():
	launchapp('mate-dictionary')
	activatewindow('frmDictionary')
	res = waittillguiexist('frmDictionary')
	print res

	if res==1:
	 	mylog.ilog(g_tag,'i catch the window')
		Sshot.scrprint(g_tag, 'After_Dictionary', './')
		sleep(5)
		print 'will be close the window'
		closewindow('frmDictionary')
